...
export default function CartItem({ item, onUpdateQuantity, onRemove }: CartItemProps) {
    ...
    return (
        <div className="group relative bg-surface-dark rounded-xl p-4 md:p-6 shadow-sm border border-surface-highlight hover:border-primary/30 transition-all duration-300">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                {/* Product Info */}
                <div className="md:col-span-6 flex gap-4">
                    <div className="shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-lg bg-background-dark overflow-hidden relative">
                        <Image
                            src={item.imageUrl}
                            alt={item.name}
                            fill
                ...
                <div className="flex justify-between items-center md:justify-center md:col-span-2">
                    ...
                    <div className="flex items-center bg-background-dark rounded-full">
                        <button
                        ...
                        <input
                            className="w-10 bg-transparent border-0 text-center text-sm font-bold text-white focus:ring-0 p-0 appearance-none"
                            type="number"
                            value={item.quantity}
                            readOnly
                        />
