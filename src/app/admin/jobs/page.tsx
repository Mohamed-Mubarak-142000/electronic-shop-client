...
export default function JobsPage() {
    ...
    const uploadFileHandler = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            setUploading(true);
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://electronic-shop-server-one.vercel.app/api'}/upload/cloudinary`, {
                    method: 'POST',
                    body: formData,
                });
                    ...
                    <tbody className="divide-y divide-border">
                        {jobs.map((job) => (
                            <tr key={job._id} className="text-text-secondary hover:bg-surface-highlight transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <Button onClick={() => handleDeleteJob(job._id)} size="icon" variant="destructive" className="rounded-full">
                    ...
                    <div className="bg-surface-dark w-full max-w-md rounded-2xl border border-border p-6 max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleCreateJob} className="space-y-4">
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                <Button
                                    type="submit"
                                    className="flex-1 rounded-full shadow-[0_0_20px_rgba(54,226,123,0.3)] hover:shadow-[0_0_30px_rgba(54,226,123,0.5)]"
                                >
